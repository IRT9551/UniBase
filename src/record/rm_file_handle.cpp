...
std::unique_ptr<RmRecord> RmFileHandle::get_record(const Rid& rid, Context* context) const {
    ...
    if (rid.slot_no < 0 || rid.slot_no >= page_handle.page_hdr->num_records || !Bitmap::is_set(page_handle.bitmap, rid.slot_no)) {
...
Rid RmFileHandle::insert_record(char* buf, Context* context) {
    ...
    while(page_no!=-1){
        bool flag=0;
        for (int slot_no = 0; slot_no < page_handle.page_hdr->num_records; ++slot_no) {
    ...
    if (free_slot == -1) {
        ...
        RmPageHandle new_page_handle = create_page_handle();
...
RmPageHandle RmFileHandle::fetch_page_handle(int page_no) const {
    ...
    int fd = GetFd();
    string table_name = disk_manager_->get_file_name(fd);
    if (page_no < 0 || page_no >= file_hdr_.num_pages) {
        throw PageNotExistError(table_name,page_no);
    }
    Page* page = buffer_pool_manager_->fetch_page((PageId){fd,page_no});
...
RmPageHandle RmFileHandle::create_new_page_handle() {
    ...
    int fd = GetFd();
    PageID page_id = (PageID){fd,INVALID_PAGE_ID};
    Page* page;
...
RmPageHandle RmFileHandle::create_page_handle() {
    ...
    int page_no = file_hdr_first_free_page_no;
